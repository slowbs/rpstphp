...
try {
  $conn = new PDO("mysql:host=$servername;dbname=$dbname;charset=utf8", $username, $password);
  $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  $stmt = $conn->prepare("SELECT form_{$ep}_$y.*, GROUP_CONCAT(ap1.valuekoon ORDER by ap1.type) as ap1,
  ...
  from form_{$ep}_$y, ap1, ap2, ap3, ap4, ap5, ap6, ap7, ap8, 
  ...
  $i=0;
    foreach($result as $row){
      if($row['status']==1){?>
        <tr>
        <td colspan="5" style="background-color : #b1b9af" class="foo"><?php echo $row['name']?></td>
        <td align="center" style="background-color : #b1b9af"><?php echo $row['koon1']?></td>
        <td align="center" style="background-color : #b1b9af"><?php echo $row['koon2']?></td>
        <td align="center" style="background-color : #b1b9af"><?php echo $row['koon3']?></td>
        <td colspan="74" style="background-color : #b1b9af"></td>
        </tr><?php
    }
    ...
    <?php
      ...
      <td></td>
        ...
        FROM total_score WHERE total_score.time = $y and ep = $ep GROUP by apid ORDER by apid"); 
        $stmt->execute();
        $result = $stmt->FetchAll(PDO::FETCH_ASSOC);
        foreach($result as $row){
          $vk = explode(",", $row["ap$i"]);
