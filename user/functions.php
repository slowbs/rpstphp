...
/* 				$query = "INSERT INTO users (username, user_type, password) 
	...
	function login(){
		...
		if (count($errors) == 0) {
			...
			$query = "SELECT users.id,users.username,users.user_type,users.password,users.hospcode,users.hospname,client.amphurcode as acode FROM users 
