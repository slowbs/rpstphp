...
try {
    ...
    $sql = "SELECT i.info_id as id, address, villagename, TAMBONNAME, amphur_name, academic, nurse, dentist,
...
<!-- <button type="button" class="btn btn-warning"><?php echo $row['updated_time']; ?></button> -->
<!-- <a href="updateuserform.php?id=<?php echo $_SESSION['user']['id']; ?>"><button type="button" class="btn btn-warning">แก้ไขรหัสผ่าน</button></a>
...
<br>
  ...
  <tbody>
    ...
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    //$sql = "SELECT * from info where hospcode = $ap;";
    $sql = "SELECT * FROM `images` where user = 'fuck'";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    $result = $stmt->FetchAll(PDO::FETCH_ASSOC);
    foreach($result as $row){
